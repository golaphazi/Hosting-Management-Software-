...
<a href="<?= SITE_URL;?>report/active_hosting_report_ajax/<?= $clientID?>/<?= $from_date?>/<?= $to_date?>?pdf_opton=csv" style="padding-left: 6px;border:none;background:none;font-size:15px;color:red;" target="_blank"> <i class="fa fa-file-excel-o" title="csv page"></i></a> 

 </div>
 <div class="col-md-12" id="divToPrint">
	<div class="page">
		<div class="subpage">
		<?= $this->user_model->select_client_information($from_date,$to_date,$clientID,'Hosting information Report');?>
	 ...
	 <?php			
		...
		if(is_array($result) AND sizeof($result) > 0){
			...
			$where = "TYPE = 'hosting' AND HISTORY_STATUS != 'Delete' AND EXPIRE_ID = '".$data->EXPIRE_ID."'";
		...
		?>
			 <tr <?= $rowClass;?> >
			  ...
			  <td><?= $domain_nae;?> (<?= sizeof($dataArray_do1) - 1; ?>) - <?= $dataArray_do[0]->DISCRIPTION;?></td>
			  <td><?= $dataArray_do[0]->DNS2; ?></td>
			  <td><?= $dataArray_do[0]->DNS2; ?></td>
			  ...
			  <td><?= $this->user_model->date_format_orginal($dataArray_do1[0]->END_DATE); ?> 
				  ...
				  <a href="<?= SITE_URL;?>dashboard/hosting_renew/<?= $dataArray_do[0]->DOMAIN_ID; ?>/" target="_blank" style="color:#fff;"><span class="fa fa-fw fa-plus" title="Renew"> </span></a>
				  <?php }else if($status == 'DeActive'){?>
				   <a href="<?= SITE_URL;?>dashboard/hosting/<?= $dataArray_do[0]->DOMAIN_ID; ?>/" target="_blank" style="color:#fff;"><span class="fa fa-fw fa-edit" title="Active"> </span></a>
				  <?php }?>
		...
		<?php
			...
			echo '<tr><td colspan="9"> No entry hosting information</td></tr>';
		}
	  ?>
	 </tbody>
	 <tfoot>
		<tr>
			  ...
			  <th colspan="8" style="text-align:right;">Total Amount := <?= number_format($total_sumT,2);?></th>			  
