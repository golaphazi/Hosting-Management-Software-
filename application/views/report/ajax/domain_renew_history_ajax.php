...
<a href="<?= SITE_URL;?>report/domain_renew_history_ajax/<?= $clientID?>/<?= $from_date?>/<?= $to_date?>?pdf_opton=csv" style="padding-left: 6px;border:none;background:none;font-size:15px;color:red;" target="_blank"> <i class="fa fa-file-excel-o" title="csv page"></i></a> 

 </div>
 <div class="col-md-12" id="divToPrint">
	<div class="page">
		<div class="subpage">
		<?= $this->user_model->select_client_information($from_date,$to_date,$clientID,'Domain Renew History');?>
	 ...
	 <?php			
		...
		if(is_array($result) AND sizeof($result) > 0){
			...
			if($clientID == 0){
				...
				$where = "TYPE = 'domain' AND DOMIAN_STATUS != 'Delete' AND CLIENTS_ID = '".$data->CLIENTS_ID."'";
				$sum = $this->user_model->count_any_where('m_domain_info', 'DOMAIN_ID', $where);
				//if($sum < $i OR $i == 1){
					echo  '<tr><td colspan="7" align="center"><strong>Client Name:</strong> '.$clientInfo[0]->CLIENTS_NAME.' ('.$clientInfo[0]->COMPANY_NAME.') </td></tr>';
				//}
			}
			 $dataArray_do = $this->user_model->renewInfo($data->DOMAIN_ID, 'domain');
			 if(is_array($dataArray_do) AND sizeof($dataArray_do) > 0){
				...
				echo  '<tr>
						...
						<td rowspan="'.$count_array.'">'.$data->DOMAIN_NAME.'  </td>
						<td rowspan="'.$count_array.'">'.$data->DOMAIN_URL.' </td>
						</tr>';
				$net_total = 0;	
				$net_total_pay = 0;	
				$net_total_due = 0;	
		...
		<?php
				...
				echo '<tr><td colspan="3"></td><td colspan="3" align="center"><strong>'.$count_renew.' y</strong></td><td align="right"><strong>'.number_format($net_total,2).'</strong></td><td align="right"><strong>'.number_format($net_total_pay,2).'</strong></td><td align="right"><strong>'.number_format($net_total_due,2).'</strong></td></tr>';
			}
			$total_sum += $net_total;
			...
			echo '<tr><td colspan="8"> No entry domain renew history</td></tr>';
		}
	  ?>
	 </tbody>
	 <?php if($clientID == 0){?>
	  <tfoot>
		 <th colspan="7" style="text-align:right;">Total Amount := <?= number_format($total_sum,2);?></th>	
